...
从10月26日开始的实验，得到了如下结果

## 分解方式1：
|压缩比|75%|90%|95%|99%|100%|原始|
|---|---|---|---|---|---|---|
|mAP(直接测试)|43.3|73.2|
|mAP(微调100轮)|
...
上一次的稀疏性实验，通过分解方式1的数据，已经验证了模型中间层在特征空间的分布稀疏性。具体表现为，仅保存输入向量的累计方差前90%主成分方向分量（大约占全部方向的20%），模型依然可以在零样本微调的情况下保持73.2的mAP。

但是，这里我们遇到了一个困难。在原本的设想当中，通过对base model进行特征空间当中的主成分分解，能够将incremental model的参数和base model严格分离，从而达到zero forgetting的增量学习。但是经过数学推导以后，我们发现，这是做不到的。所以我们又回归了9月12日的思路。经过一番调整以后，我们发现了一种新的投影损失函数实现方法，能够达到更好的增量学习效果：我们将kernel的更新值投影到由累积方差前90%的主成分所张成的子空间当中，然后直接计算更新向量和投影的余弦相似度作为损失值，不再使用方差进行缩放。并且我们为这一损失值添加了1000倍的权重。经过这样调整以后，使用了新的损失值的增量训练达到了如下效果：

## 使用伪标签蒸馏时：
||1-15|16-20|1-20|
|---|---|---|---|
|model-1|75.1|||
|model-2|65.2|68.7|55.6|
...
|---|---|---|---|
|model-1|75.1|||
|model-2|62.2|68.2|53.5|

我们发现一个很奇怪的现象：在全部1-20个类别上检测的精度，低于分别在1-15类和16-20类上的检测精度。我觉得原因可能是：我把完整的voc数据集图像分成了15:5的两个部分，其中一部分只含有1-15类的标签，另一部分只含有16-20类的标签。这就导致模型在第1部分的图像上训练了以后，在第2部分的图像上泛化性不佳，导致无法在第2部分的验证集里检测出1-15类的目标。

为此，我做了如下探究：用model-1在全部voc数据集和第2部分的数据上进行验证（带有完整的标签）。发现在cow类别得到了如下的奇怪结果。
||part 1|part 2|full|
|---|---|---|---|
|mAP|72.28|-|51.70|
|BoxP|68.12|-|44.57|
...
奇怪的点是：第2部分的图像当中完全不包含cow类别，但是在把第2部分的图像添加到验证集里面之后，cow的检测精度产生了非常大的下降。这就意味着cow类别在第2部分验证集上产生了非常大的false positive。观察BoxP和BoxR，发现从part 1迁移到full时，BoxP的下降幅度确实比BoxR要大。我们输出model-1在part 2的检测可视化结果：

![](images/val_batch1_pred-20251101.jpg)

实际情况果然如此，在第2部分的图像上可视化检测结果，发现出现了大量的cow类别误报。其中被错误检测的类别主要是：sheep。而sheep确实是在task-2当中增量的类别。经过统计我们发现，在第1部分的数据集当中，完全没有sheep类的目标。这确实解释了model-1在第2部分数据集上产生大量错误的原因。

但是为什么BoxR也在一起下降呢？按理来说，从part 1迁移到full的时候，cow类别的正样本没有增加，recall不可能会降低呀？这一点我需要再仔细查看ultralytics库的实现方式来搞明白。
...
已经进行了类增量的实验包括：VOC 15+5, VOC 15+1*5
...
15+1*5
||1-15|16|17|18|19|20|1-current|
|---|---|---|---|---|---|---|---|
|model-1|75.1||||||75.1
|model-2|64.1|47.3|||||61.1|
|model-3|50.2|20.7|81.3||||44.2|
|model-4|39.6|7.86|64.7|83.4|||33.1|
|model-5|36.1|16.9|58.4|51.6|88.7||28.9|
|model-6|27.1|17.7|35.6|39.5|63.8|72.0|22.1|

域增量实验包括：4domains(voc+clipart+watercolor+comic)，实验结果为

| 模型 | 训练数据 | VOC | CLIPART | WATERCOLOR | COMIC | 平均值 |
|------|----------|-----|---------|------------|-------|--------|
| Task-1 | VOC | **67.3%** | 26.6% | 32.8% | 11.8% | 34.6% |
| Task-2 | VOC+CLIPART | 62.3% | **40.3%** | 35.1% | 19.9% | 39.4% |
| Task-3 | VOC+CLIPART+WATERCOLOR | 60.7% | 36.1% | **40.1%** | 23.2% | 40.0% |
| Task-4 | 全部四域 | 57.4% | 35.1% | 41.9% | **29.9%** | **41.1%** |

## 2. 论文存在的问题

1. 基础模型YOLO不是主流的增量目标检测baseline，与其他工作得出的数据缺乏可比性
2. 我们的方法与Orthogonal Gradient Projection类似，但是同类型的方法没有在增量目标检测领域进行过测试，所以缺乏直接可用的对比结果
3. YOLO模型以轻量化为特点，但是我们的方法并不强调轻量化，所以选择YOLO作为基础模型是不合理的。反而我们应该选用最新的检测框架，如DINO等。

解决方案：
1. 基于我们现在的代码，实现其他Orthogonal Gradient Projection方法，进行对比。这样一来我们就获得了同样是以YOLO为基础模型的对比数据。同时目前的代码中已经包含了OSR方法的部分实现，基于此补充出完整的实现，就可以同样进行对比了。

2. 基础模型的选择，在本轮CVPR投稿当中来不及更改了，我们可以在微调阶段使用LORA技术，来强调增量学习的轻量化属性。

## 3. 方法层面存在的问题

我们一开始的思路出发点，是希望模型在增量学习阶段，权重的更新正交于特征分布较为集中的子空间，从而实现对旧任务低干扰的权重增量。

在这个基础上，我们通过绘制特征的PCA分析结果，以及模型压缩，验证了特征在整个表征空间当中具有分布稀疏性。

到了这一步，我们开始思考是否存在一种方式，利用上模型压缩得到的实验结果。将来自历史任务的模型权重和新任务的模型权重，无损地合并为一个模型权重。这一步没有得到可行的结果。

于是我们回归了最开始的设想：用方差对权重更新在各个主成分方向的投影长度进行加权求和，得到基于方差缩放的投影损失函数。一开始的实验显示，仅仅使用方差进行加权，对于权重与特征分布子空间的正交性约束不强。于是我们取消了方差加权，直接将主成分按照方差大小排序，选择方差累积值90%分位数之前的主成分，张成特征子空间，并以权重更新在该子空间的投影长度作为损失值。而且，我们还对这个损失值添加了1000倍的系数。这样我们得到了足够强的约束，使得权重更新彻底正交于特征子空间。但是，我们依然没有达到理想中的增量学习效果。具体而言，表现为：**无法找到一个平衡点，使得旧任务和新任务的检测精度，都超过对比的OSR方法。**

**另外，还有一个颇为奇怪的现象是，增量后的模型，在新旧任务样本合并起来的，带有完整标签的验证集上，取得的结果显著地低于其在两个任务各自验证集上测试的结果。** 这个奇怪的现象有如下几种可能性：
1. model-1在task-2的验证集上，对于task-1类别存在false positive
    
    1.1. model-1在task-2的验证集上，会把背景检测成task-1的类别（model-1在task-2验证集上，输出的检测结果中background占比多少，对比model-1在task-1验证集上输出结果中background的占比）
    
    1.2. model-1在task-2的验证集上，会把其他目标检测成task-1的类别（model-1在task-2验证集上，输出的检测结果中new class占比多少）

2. model-1在task-2的验证集上，对于task-1类别存在false negative
    
    2.1. model-1在task-2的验证集上，对于task-1类别检测不出来（model-1在task-2验证集上，old class被识别为background的比例是多少，对比model-1在task-1验证集上old class被识别为background的比例）
    
    2.2. model-1在task-2的验证集上，对于task-1类别会检测成其他类别（model-1在task-2验证集上，old class被识别为new class的比例是多少）

3. model-2在task-1的验证集上，对于task-2类别存在false positive
    
    3.1. model-2在task-1的验证集上，会把背景检测成task-2的类别（model-2在task-1验证集上，输出的检测结果中background占比多少，对比model-2在task-2验证集上，输出的检测结果中background的占比）
    
    3.2. model-2在task-1的验证集上，会把其他目标检测成task-2的类别（model-2在task-1验证集上，输出的检测结果中old class占比多少）

4. model-2在task-1的验证集上，对于task-2类别存在false negative
    
    4.1. model-2在task-1的验证集上，对于task-2类别检测不出来（model-2在task-1验证集上，new class被识别为background的比例是多少，对比model-2在task-2验证集上，new class被识别为background的比例是多少）
    
    4.2. model-2在task-1的验证集上，对于task-2类别会检测成其他类别（model-2在task-1验证集上，new class被识别为old class的比例是多少）

以上几种情况，均需要我们通过分别测试，然后查看confusion matrix来做判断。
