...
from .conv import Conv, DWConv, DecomposeConv
...
class DetectDecomposed(nn.Module):
    """
    YOLO Detect head for object detection models (Implemented with decomposed convolutions).
    ...
    def __init__(self, nc: int = 80, ch: Tuple = (), decomposition_args:dict=None):
        """
        Initialize the YOLO detection layer with specified number of classes and channels.
        ...
        self.cv2 = nn.ModuleList(
            nn.Sequential(
                DecomposeConv(x, c2, 3, cr=decomposition_args[f"cr_cv2_1_{i+1}"], linear_first=decomposition_args["linear_first"]),
                DecomposeConv(c2, c2, 3, cr=decomposition_args[f"cr_cv2_2_{i+1}"], linear_first=decomposition_args["linear_first"]),
                nn.Conv2d(c2, decomposition_args[f"cr_cv2_3_{i+1}"], 1, bias=False),
                nn.Conv2d(decomposition_args[f"cr_cv2_3_{i+1}"], 4 * self.reg_max, 1)) for i, x in enumerate(ch)
        )
        
        if not self.legacy:
            LOGGER.warning("Only legacy mode is supported for decomposed convolutions, but you are using non-legacy mode. This \
            will result in cv3 not being decomposed.")
        self.cv3 = (
            nn.ModuleList(nn.Sequential(
                DecomposeConv(x, c3, 3, cr=decomposition_args[f"cr_cv3_1_{i+1}"], linear_first=decomposition_args["linear_first"]),
                DecomposeConv(c3, c3, 3, cr=decomposition_args[f"cr_cv3_2_{i+1}"], linear_first=decomposition_args["linear_first"]),
                nn.Conv2d(c3, decomposition_args[f"cr_cv3_3_{i+1}"], 1, bias=False),
                nn.Conv2d(decomposition_args[f"cr_cv3_3_{i+1}"], self.nc, 1)) for i, x in enumerate(ch))
            if self.legacy
            else nn.ModuleList(
                # TODO: Figure out how to decompose the DWConv
                nn.Sequential(
                    nn.Sequential(DWConv(x, x, 3), Conv(x, c3, 1)),
                    nn.Sequential(DWConv(c3, c3, 3), Conv(c3, c3, 1)),
                    nn.Conv2d(c3, self.nc, 1),
