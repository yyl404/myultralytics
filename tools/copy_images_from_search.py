#!/usr/bin/env python3
"""Copy images into a label-only YOLO tree from one or more search roots.

Same matching logic as symlink_images_from_search.py, but writes real file
copies instead of symlinks:

    <ref>/.../labels/<split>/<stem>.txt
  -> copy found image to <ref>/.../images/<split>/<stem>.<ext>

Example:
  python tools/copy_images_from_search.py \\
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Copy images into a label-only YOLO reference tree."
    )
    parser.add_argument(
        "--ref-dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="Number of worker threads for indexing and copying (default: %(default)s).",
    )
    parser.add_argument(
        "--force",
        action="store_true",
...
def copy_image(src: Path, dst: Path, *, force: bool, dry_run: bool) -> str:
    """Copy src to dst. Returns status: created|exists|replaced|skipped."""
    if dst.exists() or dst.is_symlink():
        same_file = False
        try:
            same_file = dst.resolve() == src.resolve() and not dst.is_symlink()
        except OSError:
            same_file = False
        if same_file:
            return "exists"
        # Same size + mtime is a cheap "already copied" check for regular files.
        if (
            dst.is_file()
...
def _copy_one(
    ...
    *,
    ...
    src = candidates[0]
    dst = label_to_image_dir(label_path) / f"{stem}{src.suffix.lower()}"
    try:
        status = copy_image(src, dst, force=force, dry_run=dry_run)
...
def _print_summary(
    *,
    ...
    if failures:
        print(f"\nFailed copies, showing up to 10/{len(failures)}:")
        for item in failures[:10]:
            print(f"  {item['dst']} <- {item['src']}: {item['error']}")
...
def main() -> int:
    ...
    print(f"Copying with {args.workers} workers...")
    ...
    worker = partial(_copy_one, index=index, force=args.force, dry_run=args.dry_run)
    chunksize = max(32, len(labels) // (args.workers * 32) or 1)
    with ThreadPoolExecutor(max_workers=args.workers, thread_name_prefix="copy") as pool:
        for result in tqdm(
