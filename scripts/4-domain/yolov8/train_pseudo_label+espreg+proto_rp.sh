...
OUTPUT_DIR="runs/yolov8l_4-domain_fromscratch_pseudo_label+espreg+proto_rp"
EPOCHS=1
BATCH_SIZE=16
IMGSZ=640
WORKERS=8
...
PRORP_USE_BASE_MODEL=${PRORP_USE_BASE_MODEL:-True}
...
for DATASET_PATH in "${TASK_DATASETS[@]}"; do
    ...
    if [ $task_num -eq 1 ]; then
        ...
        if [ -n "$PREV_PCA_CACHE" ] && [ -f "$PREV_PCA_CACHE" ]; then
            ...
            TRAIN_CMD="$TRAIN_CMD --pca_cache_path $PREV_PCA_CACHE"
        else
        ...
        if [ -n "$CONVERTED_PROTOTYPES" ] && [ -f "$CONVERTED_PROTOTYPES" ]; then
            ...
            # Add prorp_use_base_model option if specified
            if [ "$PRORP_USE_BASE_MODEL" = "True" ] || [ "$PRORP_USE_BASE_MODEL" = "true" ]; then
                TRAIN_CMD="$TRAIN_CMD --prorp_use_base_model True"
        ...
        PREV_MODEL="$TASK_DIR/best.pt"
        PREV_PCA_CACHE="$PCA_CACHE_PATH"
        # Note: We don't generate new prototypes for subsequent tasks
        # Only task 1 generates prototypes, which are then converted for later tasks
    fi
