...
# Model Config
MODEL_NAME= #Fill in your model
MODEL_WEIGHT_NAME= # Fill in the pretrained weight (None for training from scratch)

# Dataset Config
DATASET_NAME= # Fill in the dataset name
CLASS_SPLITS=(
    # Fill in class splits in a sequence form
)
...
for DATASET_PATH in "${TASK_DATASETS[@]}"; do
    ...
    if [ $task_num -eq 1 ]; then
        ...
        run_step "train task ${task_num}" python tools/train.py --model $MODEL_CFG \
            --data $DATASET_PATH \
        ...
        log_info "Training for task $task_num"
        run_step "train task ${task_num}" python tools/train.py --model $EXPANDED_MODEL \
