...
OUTPUT_DIR="runs/yolov8l_voc_15_5_fromscratch_pseudo_label+nsgp"
EPOCHS=30
BATCH_SIZE=8
IMGSZ=640
WORKERS=8
...
for DATASET_PATH in "${TASK_DATASETS[@]}"; do
    ...
    if [ $task_num -eq 1 ]; then
        ...
        # Add NSGP
        TRAIN_CMD="$TRAIN_CMD --nsgp True --pca_cache_path $PREV_PCA_CACHE --optimizer SGD --weight_decay 0.0 --nsgp_flexibility 0.99" # NSGP must use SGD optim and with no weight decay
        # TRAIN_CMD="$TRAIN_CMD --espreg True --espreg_loss_weight 0.0"
