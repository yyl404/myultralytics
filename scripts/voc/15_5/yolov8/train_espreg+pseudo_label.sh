...
OUTPUT_DIR="runs/yolov8l_voc_15_5_fromscratch_pseudo_label+espreg"
EPOCHS=100
BATCH_SIZE=8
IMGSZ=640
WORKERS=8
...
# ESPReg Configuration
# Can be overridden via command line: ESPREG_LOSS_WEIGHT=1000.0 bash train_pseudo_label+espreg.sh
...
if [ $START_TASK -gt 1 ]; then
    ...
    if [ ! -f "$PREV_PCA_CACHE" ]; then
        ...
        echo "Training will proceed without PCA cache (ESPReg)."
    fi
