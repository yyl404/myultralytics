#!/bin/bash
# Feature drift between two checkpoints on the task-1 sample set of VOC 5+5+5+5.
# The dataset is fixed for this split; the two model checkpoints are passed in.
# Usage: bash scripts/voc/5_5_5_5/feature_drift.sh MODEL1_CKPT MODEL2_CKPT [SAVE_PATH]
# Example: bash scripts/voc/5_5_5_5/feature_drift.sh \
...
python tools/feature_drift.py \
    --data "data/VOC_5+5+5+5/task_1_cls_5/dataset.yaml" \
    --model1 "$MODEL1" \
    --model2 "$MODEL2" \
    --save_path "$SAVE_PATH"
