#!/bin/bash
# Feature drift between two checkpoints on the task-1 sample set of COCO 40+40.
# The dataset is fixed for this split; the two model checkpoints are passed in.
# Usage: bash scripts/coco/40_40/feature_drift.sh MODEL1_CKPT MODEL2_CKPT [SAVE_PATH]
# Example: bash scripts/coco/40_40/feature_drift.sh \
...
python tools/feature_drift.py \
    --data "data/COCO_40+40/task_1_cls_40/dataset.yaml" \
    --model1 "$MODEL1" \
    --model2 "$MODEL2" \
    --save_path "$SAVE_PATH"
