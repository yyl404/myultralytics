...
OUTPUT_DIR="runs/yolov8l_coco_40_40_fromscratch_pseudo_label+espreg"
EPOCHS=300
BATCH_SIZE=16
IMGSZ=640
WORKERS=8
...
for DATASET_PATH in "${TASK_DATASETS[@]}"; do
    ...
    if [ $task_num -eq 1 ]; then
        ...
        if [ -n "$PREV_PCA_CACHE" ] && [ -f "$PREV_PCA_CACHE" ]; then
            ...
            echo "Warning: No PCA cache available from previous task, training without ESPReg loss."
        fi
